...
bool CTriangle::Init(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pContext)
{
	...
	HRESULT hr;
	D3DX11CompileFromFile(_T("test.hlsl"), NULL, NULL, "vs_main", "vs_4_0", 0, 0, NULL, &pVertexShader, &pErrorMsg, &hr);
	if (FAILED(hr))
		assert(false);
	...
	D3DX11CompileFromFile(_T("test.hlsl"), NULL, NULL, "ps_main", "ps_4_0", 0, 0, NULL, &pPixelShader, &pBlob_Errors, &hr);
