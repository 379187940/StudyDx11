...
void FontClass::BuildVertexArray(void* vertices, char* sentence, float drawX, float drawY)
{
	...
	for(i=0; i<numLetters; i++)
	{
		...
		else
		{
			...
			vertexPtr[index].position = float3(drawX, drawY, 0.0f);  // Top left.
			vertexPtr[index].texture = float2(m_Font[letter].left, 0.0f);
			index++;

			vertexPtr[index].position = float3((drawX + m_Font[letter].size), (drawY - m_fontHeight), 0.0f);  // Bottom right.
			vertexPtr[index].texture = float2(m_Font[letter].right, 1.0f);
			index++;

			vertexPtr[index].position = float3(drawX, (drawY - m_fontHeight), 0.0f);  // Bottom left.
			vertexPtr[index].texture = float2(m_Font[letter].left, 1.0f);
			index++;
			...
			vertexPtr[index].position = float3(drawX + m_Font[letter].size, drawY, 0.0f);  // Top right.
			vertexPtr[index].texture = float2(m_Font[letter].right, 0.0f);
			index++;
