...
bool CCube::Init(ID3D11Device * pd3dDevice, ID3D11DeviceContext * pContext)
{
	...
	D3DX11CompileFromFile(_T("cube.hlsl"), NULL, NULL, "vs_main", "vs_4_0", 0, 0, NULL, &pVertexShader, NULL, &hr);
	assert(SUCCEEDED(hr));
	...
	D3DX11CompileFromFile(_T("cube.hlsl"), NULL, NULL, "ps_main", "ps_4_0", 0, 0, NULL, &pPixelShader, NULL, &hr);
	assert(SUCCEEDED(hr));
	//pixel shader
